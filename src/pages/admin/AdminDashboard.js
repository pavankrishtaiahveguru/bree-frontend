...
import { ShoppingBag, Users, MessageSquare, Clock, ArrowUpRight, ArrowDownRight, Package, TrendingUp } from 'lucide-react';
...
const StatCard = ({ icon: Icon, label, value, prefix = '', suffix = '', note, color, delay }) => {
  const [visible, setVisible] = useState(false);
  const ref = useRef();
  const count = useCounter(typeof value === 'number' ? value : 0, 1400, visible);
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay, duration: 0.5 }}
      ...
      {note && (
        <p className="text-xs text-bree-text-secondary mt-3">{note}</p>
      )}
    </motion.div>
  );
};
...
const OrderRow = ({ order, delay }) => {
  ...
  return (
    <motion.tr initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay }}
      className="border-b border-bree-border last:border-0 hover:bg-bree-bg/50 transition-colors">
      <td className="py-3 px-4 text-sm font-medium text-bree-primary">#{order.order_id || order.id}</td>
      <td className="py-3 px-4 text-sm text-bree-text-primary">{order.customer_name}</td>
      <td className="py-3 px-4 text-sm text-bree-text-secondary">{order.product_name || order.items?.[0]?.name || 'Unknown product'}</td>
      <td className="py-3 px-4 text-sm font-medium text-bree-text-primary">₹{Number(order.amount).toLocaleString()}</td>
      <td className="py-3 px-4">
        <span className={`text-xs font-medium px-2.5 py-1 rounded-full capitalize ${statusColors[order.order_status?.toLowerCase()] || 'bg-gray-100 text-gray-600'}`}>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setStats({ total_orders: 0, total_revenue: 0, total_customers: 0, pending_orders: 0 });
        setRecentOrders([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <AdminLayout>
      <div className="space-y-8">
        {/* Header */}
        <div>
          <h1 className="font-outfit text-2xl font-semibold text-bree-text-primary">Dashboard</h1>
          <p className="text-bree-text-secondary text-sm mt-1">Welcome back, Admin. Here's what's happening.</p>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="bg-white rounded-2xl p-6 animate-pulse border border-bree-border">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          ...
          <div className="px-6 py-5 border-b border-bree-border flex items-center justify-between">
            <div>
              <h2 className="font-outfit font-semibold text-bree-text-primary">Recent Orders</h2>
            ...
            <Link to="/admin/orders" className="text-sm text-bree-primary font-medium hover:underline flex items-center gap-1">
          ...
          <div className="overflow-x-auto">
            {recentOrders.length === 0 ? (
              <div className="min-h-[260px] flex flex-col items-center justify-center rounded-3xl border border-dashed border-bree-border bg-white p-10 text-center">
                ...
                <h3 className="text-xl font-semibold text-bree-text-primary mb-2">No orders yet</h3>
                ...
                <thead>
                  <tr className="bg-bree-bg/50">
                    {['Order ID', 'Customer', 'Product', 'Amount', 'Status'].map((h) => (
                      <th key={h} className="py-3 px-4 text-left text-xs font-semibold text-bree-text-secondary uppercase tracking-wide">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {recentOrders.map((order, i) => (
                    <OrderRow key={order.id || i} order={order} delay={0.5 + i * 0.05} />
                  ))}
                </tbody>
              </table>
            )}
